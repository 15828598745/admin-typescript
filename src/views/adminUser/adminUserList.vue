<template>
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="createDate" label="建立时间">
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button @click="edit(scope.row)" type="text" size="small">编辑</el-button>
          <el-popconfirm @onConfirm="del(scope.row)" confirmButtonText='确认' cancelButtonText='不用了' icon="el-icon-info" iconColor="red" title="确认删除这条数据？">
    ...
    <el-dialog title="操作" :visible.sync="dialogVisible" @closed="cancel">
      <el-form :model="form" :rules="fromRules" ref="fromRules" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="页面权限">
          <el-tree :data="treeData" show-checkbox node-key="name" :default-checked-keys="treeChecked" ref="tree" :props="treeProp" highlight-current default-expand-all>
      ...
      if (item.children) {
        ...
        for (let j = 0; j < UserModule.power.length; j++) {
          ...
          if (jitem.name === item.name) {
            if (
              jitem.children &&
              jitem.children.length === item.children.length
            ) {
              arr.push(item.name);
            }
            break;
